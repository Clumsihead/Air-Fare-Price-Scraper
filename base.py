...
class BaseAirfareAdapter(ABC):
    ...
    def run(self, search: SearchConfig) -> FeasibilityReport:
        ...
        if home_decision.fetch_error:
            # Network error / timeout: cannot confirm rules either way.
            # Log clearly, proceed so the browser can produce real evidence.
            report.robots_txt_status = "fetch_failed_proceeding_with_warning"
            report.notes.append(
                f"ROBOTS.TXT FETCH FAILED: {home_decision.fetch_error}. "
                f"Browser run will proceed to collect evidence, but this source must "
                f"be manually reviewed against its robots.txt before any repeated use."
            )
            report.log("robots_check_home", "warning", home_decision.matched_rule_hint or "")
            self.log.warning("robots.txt unreachable (%s) — proceeding with browser run, "
                             "manual review required.", home_decision.fetch_error)
        elif not home_decision.allowed:
            # Explicit Disallow rule matched — hard stop.
            report.robots_txt_status = "disallowed"
            ...
            report.suitability_reason = (
                f"robots.txt explicitly disallows the entry URL ({self.home_url}) for "
                f"user-agent '{self.cfg.user_agent}'. Per project policy this source is "
                f"not automated further without written permission from the site owner."
            )
            report.finish()
            self._persist_report(report)
            return report
        else:
            report.robots_txt_status = "allowed"
